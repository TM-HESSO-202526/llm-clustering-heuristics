...
class ClusteringHeuristic:
    def __call__(self, X, p, rng=None):
        ...
        for _ in range(7):  # Increased number of repair iterations
            high_radius_indices = np.argsort(radii)[::-1][:max(1, p // 2)]  # Increased number of high-radius clusters to repair
