    ...
    def __call__(self, X, p, rng=None):
        ...
        for _ in range(7):
            ...
            for i in range(p):
                ...
                if points_assigned_to_i.size > 0:
                    centers[i] = np.mean(points_assigned_to_i, axis=0) + rng.normal(0, 0.05, size=X.shape[1])
        ...
        for _ in range(3):
            ...
            for i in range(p):
                points_assigned_to_i = X[assignments == i]
                if points_assigned_to_i.size > 0:
                    centers[i] = np.mean(points_assigned_to_i, axis=0) + rng.normal(0, 0.01, size=X.shape[1])
